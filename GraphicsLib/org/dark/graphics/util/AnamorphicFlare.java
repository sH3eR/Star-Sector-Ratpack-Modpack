...
public class AnamorphicFlare {
    ...
    public static void createStripFlare(ShipAPI origin, Vector2f point, CombatEngineAPI engine, float brightness,
        ...
        for (int i = 0; i < magnitude; i++) {
            float angleP = angle + 90f + (float) Math.random() * 2f * spread - spread;
            if (angleP >= 360f) {
                angleP -= 360f;
            } else if (angleP < 0f) {
                angleP += 360f;
            ...
            Vector2f location = new Vector2f(point);
            float wamt = width * MathUtils.getRandomNumberInRange(-1f, 1f);
            location.x += FastTrig.cos(angleP * Math.PI / 180f) * (thickness + 4f + wamt);
            location.y += FastTrig.sin(angleP * Math.PI / 180f) * (thickness + 4f + wamt);
