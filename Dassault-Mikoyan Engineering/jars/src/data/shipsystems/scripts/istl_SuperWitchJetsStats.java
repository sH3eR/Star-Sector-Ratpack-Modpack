...
public class istl_SuperWitchJetsStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if (state == State.IN) {
            //Top Speed
            ship.getMutableStats().getMaxSpeed().modifyFlat(id, 400f);
            ship.getMutableStats().getAcceleration().modifyFlat(id, 6000f);
            ship.getMutableStats().getDeceleration().modifyFlat(id, 4800f);
            //Turning
            ship.getMutableStats().getTurnAcceleration().modifyFlat(id, 800f);
            ship.getMutableStats().getMaxTurnRate().modifyFlat(id, 240f);
