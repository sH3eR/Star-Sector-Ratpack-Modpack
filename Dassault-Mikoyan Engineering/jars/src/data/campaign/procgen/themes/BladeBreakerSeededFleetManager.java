...
import data.campaign.ids.istl_Drops;
		...
		public FIDConfig createConfig() {
			...
			config.delegate = new FIDDelegate() {
				public void postPlayerSalvageGeneration(InteractionDialogAPI dialog, FleetEncounterContext context, CargoAPI salvage) {
					...
					String [] groups = new String [] {istl_Drops.SIGMA_MATTER_ADV,
                                                                          istl_Drops.BREAKER_WEAPONS_SM,
                                                                          istl_Drops.BREAKER_WEAPONS_MED,
                                                                          istl_Drops.BREAKER_WEAPONS_LG,
                                                                          Drops.AI_CORES3};
...
//				fleet.setLocation(loc.x, loc.y);
			...
			fleet.addScript(new BladeBreakerAssignmentAI(fleet, system, null));
		}
...
//			fleet.addAssignment(FleetAssignment.HOLD, null, 1000000f, "dormant");
	...
	public static void addBladeBreakerInteractionConfig(CampaignFleetAPI fleet) {
		fleet.getMemoryWithoutUpdate().set(MemFlags.FLEET_INTERACTION_DIALOG_CONFIG_OVERRIDE_GEN, 
				   new BladeBreakerFleetInteractionConfigGen());		
	}
	...
	public static void addBladeBreakerLootDrops(Random random, CampaignFleetAPI fleet, float mult) {
		...
		int [] counts = new int[5];
		String [] groups = new String [] {istl_Drops.BREAKER_FIGHTERS,
                                                  istl_Drops.SIGMA_MATTER_PRIM,
                                                  istl_Drops.BREAKER_WEAPONS_SM,
                                                  istl_Drops.BREAKER_WEAPONS_MED,
                                                  Drops.AI_CORES3};
		for (FleetMemberAPI member : fleet.getFleetData().getMembersListCopy()) {
