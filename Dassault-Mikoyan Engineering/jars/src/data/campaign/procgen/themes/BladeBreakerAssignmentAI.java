...
public class BladeBreakerAssignmentAI implements EveryFrameScript {
	...
	public BladeBreakerAssignmentAI(CampaignFleetAPI fleet, StarSystemAPI homeSystem, SectorEntityToken source) {
		this.fleet = fleet;
	...
	protected void giveInitialAssignments() {
		...
		if (playerInSameLocation && (float) Math.random() < 0.1f && source != null) {
			...
			SectorEntityToken target = BladeBreakerSeededFleetManager.pickEntityToGuard(new Random(), homeSystem, fleet);
			if (target != null) {
