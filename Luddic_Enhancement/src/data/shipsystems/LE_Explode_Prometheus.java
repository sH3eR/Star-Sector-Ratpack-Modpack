...
public class LE_Explode_Prometheus extends BaseShipSystemScript {
...
//        }
        ...
        if ((state == State.IN && !exploded)) {
            ...
            if (interval.intervalElapsed() && ((Math.random() * Math.random()) > (stats.getEntity().getHullLevel() * 2f))) {
                ...
                exploded = true;
                {
                    LE_Dram_Missile_Plugin.explodePrometheus(ship, effectLevel);
                }
            }
        }
        ...
        if ((state == State.ACTIVE) && !exploded) {
            exploded = true;{
                LE_Dram_Missile_Plugin.explodePrometheus(ship, 1f);
            }
        }
    }
