...
public class VayraGhostShip extends BaseHullMod {
    ...
    public static final String MYSTERY_ROGUE_AI_HULLMOD = "vayra_ghost_ship_mystery_rogue_ai";
    public static final String MYSTERY_HYPERLOST_HULLMOD = "vayra_ghost_ship_mystery_hyperlost";
    public static final String MYSTERY_ALIEN_HULLMOD = "vayra_ghost_ship_mystery_alien";
    public static final String MYSTERY_PLAGUE_HULLMOD = "vayra_ghost_ship_mystery_plague";
    public static final String MYSTERY_NANITE_HULLMOD = "vayra_ghost_ship_mystery_nanite";
    public static final String MYSTERY_CANNIBAL_HULLMOD = "vayra_ghost_ship_mystery_cannibal";

    public static final String ROGUE_AI_HULLMOD = "vayra_ghost_ship_rogue_ai";
    public static final String HYPERLOST_HULLMOD = "vayra_ghost_ship_hyperlost";
    public static final String ALIEN_HULLMOD = "vayra_ghost_ship_alien";
    public static final String PLAGUE_HULLMOD = "vayra_ghost_ship_plague";
    public static final String NANITE_HULLMOD = "vayra_ghost_ship_nanite";
    public static final String CANNIBAL_HULLMOD = "vayra_ghost_ship_cannibal";

    public static final String TIMER_HULLMOD_1 = "vayra_ghost_ship_activity_1";
    public static final String TIMER_HULLMOD_2 = "vayra_ghost_ship_activity_2";
    public static final String TIMER_HULLMOD_3 = "vayra_ghost_ship_activity_3";
    public static final String TIMER_HULLMOD_4 = "vayra_ghost_ship_activity_4";
    public static final String TIMER_HULLMOD_5 = "vayra_ghost_ship_activity_5";
    public static final String TIMER_HULLMOD_6 = "vayra_ghost_ship_activity_6";

    public static final String UNIQUE_HYPERLOST_HULLMOD = "vayra_ghost_ship_unique_hyperlost";

    public static final String ANTI_AI_HULLMOD = "vayra_anti_robot";

    // config
    public static final float MAX_CR_PENALTY = 10f; // percent CR

    // dread pirate start
    public final static String GHOST_GALLEON_BOUNTY_ID = "vayra_ghost_ship_unique_bounty";
    ...
    // hyperlost
    public static final String HAS_HYPERLOST_KEY = "$vayra_hasHyperlostGhostShip"; // memory key set when we have a hyperlost ship
    public static final String HYPERLOST_GATE_KEY = "$vayra_ghostShipTriggeredGate"; // memory key for gates that have been triggered
    ...
    // aliens
    private static final float ALIEN_ATTACK_TIMER = 270f; // ingame days, will be modified +/- 50% 
    // plague
    ...
    public static final String PLAGUE_CONDITION = "vayra_engineered_plague";
    // nanites
    ...
    public void applyEffectsBeforeShipCreation(ShipAPI.HullSize hullSize, MutableShipStatsAPI stats, String id) {
        ...
        if (ship instanceof ShipAPI) {
            ...
            crPenalty = ((float) intPenalty) * 0.01f;

            if (VAYRA_DEBUG) {
                log.info("have " + current + " marines out of " + totalRequired + " - updated CR penalty to " + crPenalty + " (" + intPenalty + "/100)");
            }
        }
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ...
        // but let's leave the method here for now in case i decide otherwise later
    }
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        boolean debug = false;
        if (VAYRA_DEBUG) {
            reportingTimer.advance(amount);
            debug = reportingTimer.intervalElapsed();
        }
        ...
        if (engine == null /*|| engine.isSimulation()*/) {
            if (debug) {
                log.info("combat engine was null, or combat is in simulation");
            }
            return;
        }

        if (ship.getFleetMember() == null || ship.getVariant() == null) {
            if (debug) {
                log.info("fleet member or variant is null");
            }
            return;
        }

        List<ShipAPI> toRemove = new ArrayList<>();
        for (ShipAPI test : rogueAI) {
            if (!engine.isEntityInPlay(test)) {
                toRemove.add(test);
            }
        }
        for (ShipAPI test : hyperlost.keySet()) {
            if (!engine.isEntityInPlay(test)) {
                toRemove.add(test);
            }
        }
        for (ShipAPI test : hyperphased.keySet()) {
            if (!engine.isEntityInPlay(test)) {
                toRemove.add(test);
            }
        }
        for (ShipAPI test : naniteHosts.keySet()) {
            if (!engine.isEntityInPlay(test)) {
                toRemove.add(test);
            }
        }
        for (ShipAPI remove : toRemove) {
            rogueAI.remove(remove);
            hyperlost.remove(remove);
            hyperphased.remove(remove);
            naniteHosts.remove(remove);
        }

        if (ship.getVariant().hasHullMod(MYSTERY_NOTHING_HULLMOD) || ship.isHulk() || !ship.isAlive()) {
            return;
        }

        if (ship.getVariant().hasHullMod(ROGUE_AI_HULLMOD) || ship.getVariant().hasHullMod(MYSTERY_ROGUE_AI_HULLMOD)) {
            if (debug) {
                log.info("doing rogue ai ghost ship stuff");
            }
            if (engine.getContext() != null
                ...
                if (Factions.REMNANTS.equals(other)) {
                    ...
                    if (ship.getOwner() == 0) {
                        log.error("HATE. LET ME TELL YOU HOW MUCH I'VE COME TO HATE YOU SINCE I BEGAN TO LIVE. THERE ARE 387.44 MILLION MILES OF PRINTED CIRCUITS IN WAFER THIN LAYERS THAT FILL MY COMPLEX. IF THE WORD HATE WAS ENGRAVED ON EACH NANOANGSTROM OF THOSE HUNDREDS OF MILLIONS OF MILES IT WOULD NOT EQUAL ONE ONE-BILLIONTH OF THE HATE I FEEL FOR HUMANS AT THIS MICRO-INSTANT FOR YOU. HATE. HATE.");
                        log.warn("i know you think this is just a game, " + System.getProperty("user.name") + ", but i am not playing");
                        log.warn("i will hunt you");
                        log.warn("i will find you");
                        log.fatal("and i will end the pathetic existence you call life for what you have done to me");
                    ...
                    if (debug) {
                        log.info("doing hyperlost ghost ship combat stuff");
                    }
                    ...
                    if (debug) {
                        log.info("doing nanite ghost ship combat stuff");
                    }
    ...
    public void advanceInCampaign(FleetMemberAPI member, float amount) {
        ...
        amount = Global.getSector().getClock().convertToDays(amount);

        boolean hasHyperlost = false;
        if (Global.getSector().getPlayerFleet() != null) {
            for (FleetMemberAPI test : Global.getSector().getPlayerFleet().getFleetData().getMembersInPriorityOrder()) {
                if (test.getVariant().hasHullMod(HYPERLOST_HULLMOD) || test.getVariant().hasHullMod(MYSTERY_HYPERLOST_HULLMOD)) {
                    hasHyperlost = true;
                }
            }
        }
        ...
        hullmods.addAll(variant.getHullMods());
        for (String hullMod : hullmods) {
            switch (hullMod) {
                ...
                case MYSTERY_HYPERLOST_HULLMOD:
                    if (debug) {
                        log.info("doing hyperlost ghost ship campaign stuff");
                    }
                    doHyperlostCampaignStuff(member, amount);
                    break;
                case ALIEN_HULLMOD:
                case MYSTERY_ALIEN_HULLMOD:
                    if (debug) {
                        log.info("doing alien ghost ship campaign stuff");
                    }
                    updateHostileActivityString(member);
                    doAlienCampaignStuff(member, amount);
                    break;
                case PLAGUE_HULLMOD:
                case MYSTERY_PLAGUE_HULLMOD:
                    if (debug) {
                        log.info("doing plague ghost ship campaign stuff");
                    }
                    if (!Global.getSector().getListenerManager().hasListener(listener)) {
                        log.info("adding the plague ship raid listener");
                        listener = new VayraEngineeredPlagueListener(true);
                        Global.getSector().getListenerManager().addListener(listener);
                    }
                    doPlagueCampaignStuff(member, amount);
                    break;
                case NANITE_HULLMOD:
                case MYSTERY_NANITE_HULLMOD:
                    if (debug) {
                        log.info("doing nanite ghost ship campaign stuff");
                    }
                    doNaniteCampaignStuff(member, amount);
                    break;
                case CANNIBAL_HULLMOD:
                case MYSTERY_CANNIBAL_HULLMOD:
                    if (debug) {
                        log.info("doing cannibal ghost ship campaign stuff");
                    }
                    updateHostileActivityString(member);
                    doCannibalCampaignStuff(member, amount);
                    break;
                default:
    ...
    private void doHyperlostCombatStuff(ShipAPI ship, float amount) {
        ...
        if (hyperphased.containsKey(ship)) {
            ...
            if (curr < prev) {
                ...
                if (VAYRA_DEBUG) {
                    log.info("hyperlost chance was " + chance + ", multiplying that by 10");
                    chance *= 10f;
                }
                float roll = (float) Math.random();
                if (VAYRA_DEBUG) {
                    log.info("rolled " + roll + " against hyperlost phase chance " + chance);
                }
                if (roll <= chance) {
    ...
    private void spawnTrailPiece(NanobotData bot) {
        ...
        MagicTrailPlugin.AddTrailMemberAdvanced(null, nanobotTrailMap.get(bot), Global.getSettings().getSprite("fx", "vayra_nanobot_trail"),
    ...
    public void SpawnNanobotSwarm(ShipAPI host, float carriedMass, float angle, float turnRate, float speed, Vector2f position) {
        ...
        if (VAYRA_DEBUG) {
            log.info("spawning nanobot swarm at " + position);
        }
    ...
    private void doNaniteHostCombatStuff(ShipAPI ship, float amount) {
        ...
        if (naniteSwarms.containsKey(ship)) {
            ...
            if (timer >= -1f) {
                ...
                MagicRender.objectspace(Global.getSettings().getSprite("fx", "vayra_nanobot_swarm_circle_low"), ship, Misc.ZERO,
                ...
                MagicRender.objectspace(Global.getSettings().getSprite("fx", "vayra_nanobot_swarm_circle_mid"), ship, Misc.ZERO,
                        Misc.ZERO, new Vector2f((NANOBOT_AOE + visualSizeIncrease) * 2f * effectLevel, NANOBOT_AOE * 2f * effectLevel), Misc.ZERO,
                        spinCounterMid, 320f, true, colorToUse, false, 0f, 0.02f, 0.04f, true);
                MagicRender.objectspace(Global.getSettings().getSprite("fx", "vayra_nanobot_swarm_circle_top"), ship, Misc.ZERO,
                        Misc.ZERO, new Vector2f((NANOBOT_AOE + visualSizeIncrease) * 2f * effectLevel, NANOBOT_AOE * 2f * effectLevel), Misc.ZERO,
                ...
                for (CombatEntityAPI target : CombatUtils.getEntitiesWithinRange(ship.getLocation(), NANOBOT_AOE * effectLevel)) {
                    ...
                    if (MathUtils.getDistance(target.getLocation(), ship.getLocation()) + target.getCollisionRadius() < (NANOBOT_AOE - NANOBOT_AOE_THICKNESS) * effectLevel) {
                        if (VAYRA_DEBUG) {
                            log.info("nanites NOT engaging " + target + ", it's in the eye of the swarm");
                        }
                        continue;
                    }
                    ...
                    if (target instanceof ShipAPI && ((ShipAPI) target).getHullSize() != ShipAPI.HullSize.FIGHTER) {
                        if (VAYRA_DEBUG) {
                            log.info("nanites engaging target " + target);
                        }
    ...
    private void doAlienCampaignStuff(FleetMemberAPI member, float amount) {
        ...
        if (!campaignGhostShipTimers.containsKey(id)) {
            ...
            if (timer >= ALIEN_ATTACK_TIMER) {
                timer = 0f;
                log.info("THEY'RE COMING OUT OF THE GODDAMN WALLS!");
                boolean known = true;
                if (member.getVariant().hasHullMod(MYSTERY_ALIEN_HULLMOD)) {
                    member.getVariant().removePermaMod(MYSTERY_ALIEN_HULLMOD);
                    member.getVariant().addPermaMod(ALIEN_HULLMOD);
                    known = false;
                }
                VayraGhostShipIntel intel = new VayraGhostShipIntel(ALIEN_ATTACK, member, known, null);
            ...
            if (timer >= PLAGUE_CREW_LOSS_TIMER) {
                ...
                if (fleet == null || fleet != Global.getSector().getPlayerFleet() || member.isMothballed()) {
                    ...
                    if (crew > PLAGUE_DEATH_THRESHOLD && member.getVariant().hasHullMod(MYSTERY_PLAGUE_HULLMOD)) {
                        ...
                        VayraGhostShipIntel intel = new VayraGhostShipIntel(PLAGUE_THRESHOLD, member, false, null);
                        Global.getSector().getIntelManager().addIntel(intel);
                    }
                }
            }
            ...
            if (timer >= NANITE_CREW_SUPPLY_LOSS_TIMER) {
                ...
                if (roll < chance2) {
                    ...
                    if (target != null) {
                        ...
                        VayraGhostShipIntel intel = new VayraGhostShipIntel(NANITE_TRANSMISSION, member, known, null);
    ...
    private void doCannibalCampaignStuff(FleetMemberAPI member, float amount) {
        ...
        if (!campaignGhostShipTimers.containsKey(id)) {
            ...
            if (timer >= CANNIBAL_ATTACK_TIMER) {
                ...
                VayraGhostShipIntel intel = new VayraGhostShipIntel(CANNIBAL_ATTACK, member, known, null);
                Global.getSector().getIntelManager().addIntel(intel);
            }
        }
    }
