...
public class VayraLRMAI implements MissileAIPlugin, GuidedMissileAI {

    //     SETTINGS     //
    private final float OVERSHOT_ANGLE = 15f;      // Angle with the target beyond which the missile turn around without accelerating. Avoid endless circling. <0 = disabled
    private final float WAVE_TIME = 1.5f;            // Time to complete a wave in seconds.
    private final float WAVE_AMPLITUDE = 75f;      // Max angle of the waving in degree (divided by 3 with ECCM). Set to a negative value to avoid all waving.
    ...
    private final int MAX_SEARCH_RANGE = 10000;  // range in which the missile seek a target in game units.
    ...
    private float PRECISION_RANGE = 1000f;          // range under which the missile start to get progressively more precise in game units.
    ...
    private float ECCM = 4;   // A VALUE BELOW 1 WILL PREVENT THE MISSILE FROM EVER HITTING ITS TARGET!
    ...
    private final int destroyers = 7;                     //Target class priorities, 0 = ignore
    private final int cruisers = 10;                       //Target class priorities, 0 = ignore
    ...
    public VayraLRMAI(MissileAPI missile, ShipAPI launchingShip) {
