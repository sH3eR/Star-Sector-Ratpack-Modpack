...
public class VayraAntiFighterAI implements MissileAIPlugin, GuidedMissileAI {
    ...
    private final float WAVE_TIME = 0.5f;            // Time to complete a wave in seconds.
    private final float WAVE_AMPLITUDE = 120f;      // Max angle of the waving in degree (divided by 3 with ECCM). Set to a negative value to avoid all waving.
    private final float DAMPING = 1f;           // Damping of the turn speed when closing on the desired aim. The smaller the snappier.
    private final boolean TARGET_SWITCH = false;   // Does the missile switch its target if it has been destroyed?
    ...
    public VayraAntiFighterAI(MissileAPI missile, ShipAPI launchingShip) {
