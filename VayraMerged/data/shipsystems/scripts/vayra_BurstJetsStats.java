...
public class vayra_BurstJetsStats extends BaseShipSystemScript {
    ...
    public StatusData getStatusData(int index, State state, float effectLevel) {
        if (index == 1) {
            return new StatusData("zoom zoom", false);
        }
        if (index == 0) {
            return new StatusData("they'll never take me alive", false);
        }
        return null;
    }
}
