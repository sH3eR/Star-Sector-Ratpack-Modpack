...
class CrewConversionPrimordialRefitButton : BaseRefitButton() {
    ...
    override fun shouldShow(member: FleetMemberAPI?, variant: ShipVariantAPI?, market: MarketAPI?): Boolean {
        return variant!!.hasHullMod("rat_abyssal_conversion") && getPrimordialStack(market) != null
    }
    ...
    override fun addTooltip(tooltip: TooltipMakerAPI?,member: FleetMemberAPI?, variant: ShipVariantAPI?, market: MarketAPI?) {
        ...
        var label = tooltip!!.addPara("Double-clicking this button installs a primordial core into the crew-converted Abyssal hull.", 0f,
        Misc.getTextColor(), Misc.getHighlightColor(), "Double-Clicking", "primordial core")

        tooltip.addSpacer(5f)

        tooltip.addPara("This allows the ship to use the primordial core's signature skill." +
                "", 0f,
        Misc.getTextColor(), Misc.getHighlightColor(), "shipsystem", "permanently")

        if (getPrimordialStack(market) == null) {
            tooltip.addSpacer(5f)
            tooltip.addNegativePara("No Primordial Core in inventory.")
        }
    }

    override fun isClickable(member: FleetMemberAPI?, variant: ShipVariantAPI?, market: MarketAPI?): Boolean {
        return getPrimordialStack(market) != null
    }
    ...
    override fun onClick(member: FleetMemberAPI?, variant: ShipVariantAPI?, event: InputEventAPI?, market: MarketAPI?) {
        ...
        var primordialStack = getPrimordialStack(market)

        if (event!!.isLMBEvent && event.isDoubleClick && primordialStack != null) {
            variant!!.removeMod("rat_abyssal_conversion")
            variant!!.addMod("rat_primordial_conversion")

            Global.getSoundPlayer().playUISound("ui_char_spent_story_point", 1f, 1f)

            reduceOrRemoveStack(primordialStack)
            refreshVariant()
            refreshButtonList()
        }
    ...
    fun getPrimordialStack(market: MarketAPI?) : CargoStackAPI? {
        var stack = Global.getSector().playerFleet.cargo.stacksCopy.find { it.isCommodityStack && it.commodityId == RATItems.PRIMORDIAL }
        if (stack != null) return stack

        if (market != null && market.getStorage() != null) {
            stack = market.getStorage().cargo.stacksCopy.find { it.isCommodityStack && it.commodityId == RATItems.PRIMORDIAL }
        }
        return stack
    }
