...
public class HMI_MoteControlScript extends BaseShipSystemScript {
...
//	}
	...
	public static String MOTELAUNCHER = "hmi_motelauncher";
	public static String MOTELAUNCHER_HF = "hmi_motelauncher_hf";
	...
	public static class SharedHMIMoteAIData {
	...
	public static SharedHMIMoteAIData getSharedData(ShipAPI source) {
		String key = source + "_mote_AI_shared";
		SharedHMIMoteAIData data = (SharedHMIMoteAIData)Global.getCombatEngine().getCustomData().get(key);
		if (data == null) {
			data = new SharedHMIMoteAIData();
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		SharedHMIMoteAIData data = getSharedData(ship);
		data.elapsed += amount;
		...
		if (effectLevel == 1) {
			...
			EmpArcEntityAPI arc = (EmpArcEntityAPI)engine.spawnEmpArc(ship, slotLoc, ship, target,
							   ...
							   HMI_MoteControlScript.getEMPColor(ship),
...
//			}
	...
	protected void spawnAttractorParticles(ShipAPI ship) {
		...
		float minSize = 15f;
		float maxSize = 30f;
		Color color = HMI_MoteControlScript.getEMPColor(ship);
		for (int i = 0; i < glows; i++) {
			float radius = minRadius + (float) Math.random() * (maxRadius - minRadius);
			Vector2f loc = Misc.getPointAtRadius(targetLoc, radius);
			Vector2f dir = Misc.getUnitVectorAtDegreeAngle(Misc.getAngleInDegrees(loc, targetLoc));
	...
	public void calculateTargetData(ShipAPI ship) {
		...
		if (data.attractorLock != null) {
			...
			Global.getCombatEngine().addPlugin(createTargetJitterPlugin(data.attractorLock,
					...
					HMI_MoteControlScript.getJitterColor(ship)));
		}
	}
