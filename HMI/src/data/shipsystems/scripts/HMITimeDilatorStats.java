...
public class HMITimeDilatorStats extends BaseShipSystemScript {
	public static final Object KEY_SHIP = new Object();
	public static final Object KEY_TARGET = new Object();

	public static final float DAM_MULT = 0.25f;
	public static final float RANGE = 2000f;

	public static final Color TEXT_COLOR = new Color(145, 100, 255,255);

	public static final Color JITTER_COLOR = new Color(145, 100, 255,150);
	public static final Color JITTER_UNDER_COLOR = new Color(149, 125, 255,155);
	...
	public void apply(MutableShipStatsAPI stats, final String id, State state, float effectLevel) {
		...
		final String targetDataKey = ship.getId() + "_Dilate_target_data";
		...
		if (state == State.IN && targetDataObj == null) {
			...
			if (target != null) {
				if (target.getFluxTracker().showFloaty() ||
					...
					target.getFluxTracker().showOverloadFloatyIfNeeded("Time Dilated!", TEXT_COLOR, 4f, true);
				}
			}
		...
		targetData.currDamMult = DAM_MULT * effectLevel;
			...
			targetData.targetEffectPlugin = new BaseEveryFrameCombatPlugin() {
				...
				public void advance(float amount, List<InputEventAPI> events) {
					...
					if (targetData.target == Global.getCombatEngine().getPlayerShip()) {
						Global.getCombatEngine().getTimeMult().modifyMult(id, DAM_MULT);
						Global.getCombatEngine().maintainStatusForPlayerShip(KEY_TARGET,
								...
								"" + (int)((1f - targetData.currDamMult) * 100f) + "% slower time", true);
					}

					if (targetData.currDamMult <= 0f || !targetData.ship.isAlive()) {
						targetData.target.getMutableStats().getTimeMult().unmodify(id);
						Global.getCombatEngine().getTimeMult().unmodify(id);
						Global.getCombatEngine().removePlugin(targetData.targetEffectPlugin);
					} else {
						targetData.target.getMutableStats().getTimeMult().modifyMult(id, targetData.currDamMult);
						Global.getCombatEngine().getTimeMult().unmodify(id);
					}
				}
			};
		...
		if (effectLevel > 0) {
			...
			if (shipJitterLevel > 0) {
				...
				ship.setJitter(KEY_SHIP, color, shipJitterLevel, 6, 2f, 0 + jitterRangeBonus * 1f);
			}
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		if (effectLevel > 0) {
			if (index == 0) {
				float damMult = 1f + (1f - DAM_MULT) * effectLevel;
				return new StatusData("" + (int)((damMult - 1f) * 100f) + "% slower time", false);
			}
		}
		return null;
	}
