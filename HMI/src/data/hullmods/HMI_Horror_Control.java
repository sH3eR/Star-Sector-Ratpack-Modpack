...
public class HMI_Horror_Control extends BaseHullMod {
...
//				if (yoinkTimer.intervalElapsed()) {
				// Should pick four small ships, and swap their teams

				int nightmare_smol_controlled = 0;
...
//					boolean counterPresent = false;
				...
				for (ShipAPI target : CombatUtils.getShipsWithinRange(ship.getLocation(), 10000f)) {
					if (target.getOwner() != ship.getOwner() && target.isAlive() && !target.getHullSpec().getHullId().startsWith("hmi_spooky")){
						if  (nightmare_smol_controlled >= 4)
							return;

						if (target.getHullSize().equals(HullSize.FRIGATE) || target.getHullSize().equals(HullSize.DESTROYER))
							ValidTarget = true;

						if (ValidTarget) {


							//I have no idea but apparently this can happen when new ships spawn?
							if (target == null || target.isFighter() || target.getCaptain() == null) {
								return;
							}

							// Nothing happens to the player if yoinked
