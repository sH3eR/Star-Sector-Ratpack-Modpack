...
public class HMI_MessCore extends BaseHullMod {

    private static final Map<HullSize, Integer> MAG = new HashMap<>();
    static {
        MAG.put(HullSize.FRIGATE, 0);
        MAG.put(HullSize.DESTROYER, 0);
        MAG.put(HullSize.CRUISER, 1);
        MAG.put(HullSize.CAPITAL_SHIP, 2);
    }
    ...
    public void advanceInCampaign(FleetMemberAPI member, float amount) {
        ...
        // and another
        if (!member.getFleetCommander().getFaction().getId().contains("mess")) {
            return;
        }
        ...
        if (die <= 2) {
             person = Misc.getAICoreOfficerPlugin(Commodities.GAMMA_CORE).createPerson(Commodities.GAMMA_CORE, "mess", Misc.random);
             member.setCaptain(person);
        }
        if (die > 2 ) {
             person = createPerson(hmi_Tags.MESSCORE, "mess", Misc.random);
             member.setCaptain(person);
        }
