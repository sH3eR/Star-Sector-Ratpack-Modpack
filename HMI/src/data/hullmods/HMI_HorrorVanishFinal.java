...
public class HMI_HorrorVanishFinal extends BaseHullMod {
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		...
		for (ShipAPI target : CombatUtils.getShipsWithinRange(ship.getLocation(), 100000f)) {
			if (target.getHullSpec().getHullId().startsWith("hmi_spooky") && target.isAlive()){
				Vector2f loc = new Vector2f(target.getLocation());
				engine.applyDamage(target, loc, 200000f, DamageType.HIGH_EXPLOSIVE, 0f, true, false, null);
			}
			if (!target.getHullSpec().getHullId().startsWith("hmi_spooky") && target.isAlive()) {
