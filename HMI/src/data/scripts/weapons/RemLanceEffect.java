...
public class RemLanceEffect implements BeamEffectPlugin {
		...
		if (target instanceof ShipAPI && beam.getBrightness() >= 1f) {
			...
			if (fireInterval.intervalElapsed()) {
				...
				if (!hitShield || piercedShield) {
					...
					float FLUX_LEVEL = (beam.getSource().getFluxTracker().getFluxLevel())* 0.3f + 0.5f;
					float emp = beam.getDamage().getFluxComponent() * 0.35f;
					float dam = beam.getDamage().getDamage() * FLUX_LEVEL;
