...
public class HMMaxwellFleetAssignmentAI extends RouteFleetAssignmentAI {

	protected boolean pirate;
	protected IntervalUtil piracyCheck = new IntervalUtil(0.2f, 0.4f);
	public HMMaxwellFleetAssignmentAI(CampaignFleetAPI fleet, RouteData route, boolean pirate) {
		super(fleet, route);
		this.pirate = pirate;
	}
	...
	protected void doPiracyCheck() {
		...
		if (isCurrentlyPirate) {
			...
			if (visible.isEmpty()) {
				fleet.setFaction("hmi_maxwell", true);
		...
		if (visible.size() == 1) {
			...
			for (CampaignFleetAPI other : visible) {
				if (fleet.getAI() != null && 
						Global.getSector().getFaction(Factions.REMNANTS).isHostileTo(other.getFaction())) {
