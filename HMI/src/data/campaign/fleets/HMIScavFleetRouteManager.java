...
public class HMIScavFleetRouteManager extends BaseRouteFleetManager {

	protected StarSystemAPI system;
	
	public HMIScavFleetRouteManager(StarSystemAPI system) {
		super(1f, 14f);
		this.system = system;
	}
	...
	protected int getMaxFleets() {
		...
		for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
			if (market.getFactionId().contentEquals("HMI")) {
				numHMIMarkets++;
			}
		}
...
//		}
		...
		for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
			if (market.getFaction().isHostileTo("HMI")) continue;
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		CampaignFleetAPI fleet = createHMIScavenger(type, system.getLocation(), route, route.getMarket(), pirate, random);
		if (fleet == null) return null;;
		
		fleet.addScript(new HMIScavengerFleetAssignmentAI(fleet, route, pirate));
		
		return fleet;
	}
	
	public static CampaignFleetAPI createHMIScavenger(String type, Vector2f locInHyper, MarketAPI source, boolean pirate, Random random) {
		return createHMIScavenger(type, locInHyper, null, source, pirate, random);
	}
	public static CampaignFleetAPI createHMIScavenger(String type, Vector2f locInHyper, RouteData route, MarketAPI source, boolean pirate, Random random) {
...
//			combat += utility;
		...
		FleetParamsV3 params = new FleetParamsV3(
				...
				locInHyper,
				"HMI", // quality will always be reduced by non-market-faction penalty, which is what we want
