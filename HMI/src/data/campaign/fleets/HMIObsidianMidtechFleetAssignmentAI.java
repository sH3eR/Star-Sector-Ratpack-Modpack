...
public class HMIObsidianMidtechFleetAssignmentAI extends RouteFleetAssignmentAI {

	protected boolean pirate;
	protected IntervalUtil piracyCheck = new IntervalUtil(0.2f, 0.4f);
	public HMIObsidianMidtechFleetAssignmentAI(CampaignFleetAPI fleet, RouteData route, boolean pirate) {
		super(fleet, route);
		this.pirate = pirate;
	}
	...
	protected void doPiracyCheck() {
		...
		if (visible.size() == 1) {
			...
			for (CampaignFleetAPI other : visible) {
				if (fleet.getAI() != null && 
						Global.getSector().getFaction(Factions.PERSEAN).isHostileTo(other.getFaction()) && !other.getTags().equals("ObsidianMidTech")) {
