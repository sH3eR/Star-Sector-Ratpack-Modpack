...
public class HMIObsidianHitechFleetAssignmentAI extends RouteFleetAssignmentAI {

	protected boolean pirate;
	protected IntervalUtil piracyCheck = new IntervalUtil(0.2f, 0.4f);
	public HMIObsidianHitechFleetAssignmentAI(CampaignFleetAPI fleet, RouteData route, boolean pirate) {
		super(fleet, route);
		this.pirate = pirate;
	}
	...
	protected void doPiracyCheck() {
		...
		if (visible.size() == 1) {
			...
			for (CampaignFleetAPI other : visible) {
				if (fleet.getAI() != null && 
						Global.getSector().getFaction(Factions.TRITACHYON).isHostileTo(other.getFaction()) && !other.getTags().equals("ObsidianHiTech")) {
