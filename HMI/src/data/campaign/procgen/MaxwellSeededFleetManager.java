...
public class MaxwellSeededFleetManager extends SeededFleetManager {
	...
	public MaxwellSeededFleetManager(StarSystemAPI system, int minFleets, int maxFleets, int minPts, int maxPts, float activeChance) {
		super(system, 1f);
		this.minPts = minPts;
		this.maxPts = maxPts;
	...
	protected CampaignFleetAPI spawnFleet(long seed) {
		...
		initMaxwellFleetProperties(random, fleet, dormant);
...
//				fleet.setLocation(loc.x, loc.y);
			...
			fleet.addScript(new MaxwellAssignmentAI(fleet, system, null));
		}
	...
	public static void initMaxwellFleetProperties(Random random, CampaignFleetAPI fleet, boolean dormant) {
		if (random == null) random = new Random();

		fleet.removeAbility(Abilities.GO_DARK);
		// make them act like normal patrols
