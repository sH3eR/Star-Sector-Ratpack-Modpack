...
public class MessRemnSeededFleetManager extends SeededFleetManager {

	
	public static class MessRemnantFleetInteractionConfigGen implements FIDConfigGen {
...
//					}
	...
	public MessRemnSeededFleetManager(StarSystemAPI system, int minFleets, int maxFleets, int minPts, int maxPts, float activeChance) {
		super(system, 1f);
		this.minPts = minPts;
		this.maxPts = maxPts;
	...
	protected CampaignFleetAPI spawnFleet(long seed) {
		...
		if (combatPoints > 25) type = FleetTypes.PATROL_MEDIUM;
		if (combatPoints > 45) type = FleetTypes.PATROL_LARGE;
		...
		initMessRemnantFleetProperties(random, fleet, dormant);
...
//				fleet.setLocation(loc.x, loc.y);
			...
			fleet.addScript(new MessRemnAssignmentAI(fleet, system, null));
		}
	...
	public static void initMessRemnantFleetProperties(Random random, CampaignFleetAPI fleet, boolean dormant) {
...
//			fleet.addAssignment(FleetAssignment.HOLD, null, 1000000f, "dormant");
		...
		addMessRemnantInteractionConfig(fleet);
		//addBladeBreakerAICoreDrops(random, fleet, 1f);
	}
	
	public static void addMessRemnantInteractionConfig(CampaignFleetAPI fleet) {
		fleet.getMemoryWithoutUpdate().set(MemFlags.FLEET_INTERACTION_DIALOG_CONFIG_OVERRIDE_GEN, 
				   new MessRemnantFleetInteractionConfigGen());
	}
	
//	public static void addBladeBreakerAICoreDrops(Random random, CampaignFleetAPI fleet, float mult) {
