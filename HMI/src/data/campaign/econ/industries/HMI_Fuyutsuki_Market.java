...
public class HMI_Fuyutsuki_Market extends BaseIndustry implements RouteFleetSpawner, FleetEventListener { //HMI_Fuyutsuki_Market
	protected transient SubmarketAPI saved = null;
        
	@Override
	public boolean isHidden() {
		return !market.getFactionId().equals("HMI"); //HMI
	}
	
	@Override
	public boolean isFunctional() {
		return super.isFunctional() && market.getFactionId().equals("HMI"); //HMI
	}
	...
	public void apply() {
		...
		float officerProb = 0.3f;//OFFICER_PROB_MOD_HMI;
		...
		Misc.setFlagWithReason(memory, MemFlags.MARKET_MILITARY, getModId(), true, -1);
                ...
                if (isFunctional()) {
                    SubmarketAPI fuyutsuki = market.getSubmarket("hmi_exec_market"); //hmi_exec_market
                    if (fuyutsuki == null) {
                        if (saved != null) {
                            market.addSubmarket(saved);
                        } else {
                            market.addSubmarket("hmi_exec_market"); //hmi_exec_market
	...
	public void unapply() {
		...
		SubmarketAPI fuyutsuki = market.getSubmarket("hmi_exec_market"); //hmi_exec_market
		saved = fuyutsuki;
		market.removeSubmarket("hmi_exec_market"); //hmi_exec_market
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		FleetParamsV3 params = new FleetParamsV3(
				...
				"hmi_exec",//Factions.LIONS_GUARD, "hmi_exec"
