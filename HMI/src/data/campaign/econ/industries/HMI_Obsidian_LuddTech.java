...
import data.campaign.fleets.HMIObsidianLuddtechFleetAssignmentAI;

public class HMI_Obsidian_LuddTech extends BaseIndustry implements RouteManager.RouteFleetSpawner, FleetEventListener {

	public static final WeightedRandomPicker<String> LUDDTECH_FACTIONS = new WeightedRandomPicker<>();

	static {
		LUDDTECH_FACTIONS.add(Factions.LUDDIC_CHURCH, 4f);
		LUDDTECH_FACTIONS.add(Factions.INDEPENDENT, 0.5f);
		LUDDTECH_FACTIONS.add(Factions.HEGEMONY, 0.5f);
	}
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		int index = 0;
		for (String item : LUDDTECH_FACTIONS.getItems()) {
			FactionAPI f;
			try {
			...
			if (f != null) {
				factionPicker.add(f.getId(), LUDDTECH_FACTIONS.getWeight(index));
			}
			index++;
		}
...
//		fleet.addScript(ai);
		...
		fleet.addScript(new HMIObsidianLuddtechFleetAssignmentAI(fleet, route, pirate));
		fleet.addTag("ObsidianLuddTech");
		return fleet;
	}

	public String getRouteSourceId() {
		return getMarket().getId() + "_" + "luddobsidian";
	}
