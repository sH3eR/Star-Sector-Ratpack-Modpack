...
public class SFCFuelDestabilizerStats {
    ...
    public static Color TEXT_COLOR = new Color(241, 50, 50,255);

    public static Color JITTER_COLOR = new Color(255, 2, 2,75);
    public static Color JITTER_UNDER_COLOR = new Color(199, 77, 77,155);
        ...
        if (state == State.IN && targetDataObj == null) {
            ...
            if (target != null) {
                if (target.getFluxTracker().showFloaty() ||
                    ...
                    target.getFluxTracker().showOverloadFloatyIfNeeded("Fuel Destabilized!", TEXT_COLOR, 4f, true);
                }
            }
                ...
                public void advance(float amount, List<InputEventAPI> events) {
                    ...
                    if (targetData.target == Global.getCombatEngine().getPlayerShip()) {
                        Global.getCombatEngine().maintainStatusForPlayerShip(KEY_TARGET,
                                ...
                                "" + (int)((targetData.currDamMult - 1f) * 100f) + "% more hull damage taken", true);
                    }
    ...
    public ShipSystemStatsScript.StatusData getStatusData(int index, State state, float effectLevel) {
        if (effectLevel > 0) {
            if (index == 0) {
                ...
                return new ShipSystemStatsScript.StatusData("" + (int)((damMult - 1f) * 100f) + "% more damage to target", false);
            }
        }
        return null;
    }
