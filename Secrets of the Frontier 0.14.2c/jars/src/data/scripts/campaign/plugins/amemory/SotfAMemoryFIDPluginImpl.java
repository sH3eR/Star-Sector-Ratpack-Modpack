// aka SecretsofthefrontierAMemoryFleetInteractionDialoguePluginImplementationDotJava
...
public class SotfAMemoryFIDPluginImpl extends FleetInteractionDialogPluginImpl {

    protected boolean shownTooLargeToRetreatMessage = true;

    public SotfAMemoryFIDPluginImpl() {
        this(null);
    }

    public SotfAMemoryFIDPluginImpl(FIDConfig params) {
        super(params);
        context = new SotfAMemoryFEContext();
    }

    // disabled Transfer Command - already transferred to a valid Concord ship in the rulecommand
    protected void updatePreCombat() {
        ...
        if (playerGoal == FleetGoal.ATTACK && otherGoal == FleetGoal.ESCAPE) {
            ...
            if (config.straightToEngage) {
                ...
                selectedFlagship = sierra;
                Global.getSector().getPlayerFleet().getFleetData().setFlagship(selectedFlagship);
                if (origFlagship != sierra) {
                    origFlagship.setCaptain(null);
                }
                options.addOption("Listen closer", OptionId.CONTINUE_INTO_BATTLE, null);
            } else {
