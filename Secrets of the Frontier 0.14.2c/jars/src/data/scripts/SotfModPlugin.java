...
public class SotfModPlugin extends BaseModPlugin {
    ...
    public void onGameLoad(boolean newGame) {
        ...
        if (Global.getSector().getStarSystem("sotf_mia") != null) {
            Global.getSector().getStarSystem("sotf_mia").addScript(new SotfDustkeeperObjectiveShareScript(Global.getSector().getStarSystem("sotf_mia")));
        }
        ...
        if (sector_mem.get("$sotf_kindred") == "Commander") {
            sector_mem.set("$sotf_kindred", "captain");
            sector_mem.set("$sotf_Kindred", "Captain");
        }

        // rename ??? to Wayward Star
        if (Global.getSector().getStarSystem("???") != null) {
            StarSystemAPI ws = Global.getSector().getStarSystem("???");
            ws.getStar().setName("Wayward Star");
            ws.setName("Wayward Star System");
            ws.setOptionalUniqueId("sotf_waywardstar");
            sector_mem.set("$sotf_waywardstar", ws);
        }

        // end WS intel item if already completed
        if (sector.getIntelManager().hasIntelOfClass(SotfWaywardStarIntel.class) && sector_mem.contains("$sotf_WSCombatWon")) {
            SotfWaywardStarIntel ws = (SotfWaywardStarIntel) sector.getIntelManager().getFirstIntel(SotfWaywardStarIntel.class);
            ws.endAfterDelay();
        }
