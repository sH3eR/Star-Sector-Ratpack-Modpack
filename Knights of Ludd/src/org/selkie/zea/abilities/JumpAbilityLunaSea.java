package org.selkie.zea.abilities;
...
public class JumpAbilityLunaSea extends BaseDurationAbility {
	...
	protected void applyEffect(float amount, float level) {
		...
		if (level == 1 && primed != null) {
			if (fleet.isInHyperspace() && canUseToJumpToSystem()) {
				...
				StarSystemAPI system = Global.getSector().getStarSystem(ZeaStaticStrings.lunaSeaSysName);
	...
	public void createTooltip(TooltipMakerAPI tooltip, boolean expanded) {
		...
		LabelAPI title = tooltip.addTitle("Fracture Jump to the Luna Sea");

		float pad = 10f;

		tooltip.addPara("Jump across space to the gravity well of the Luna Sea.", pad);
