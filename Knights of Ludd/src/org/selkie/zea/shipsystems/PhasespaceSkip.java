...
public class PhasespaceSkip extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        MagicRender.singleframe(Global.getSettings().getSprite(GfxCat.PHASE_GLOWS, ship.getHullSpec().getBaseHullId() + "_glow1"), spriteCenter,
                new Vector2f(ship.getSpriteAPI().getWidth(), ship.getSpriteAPI().getHeight()), ship.getFacing() - 90f, colorToUse, true);
        MagicRender.singleframe(Global.getSettings().getSprite(GfxCat.PHASE_GLOWS, ship.getHullSpec().getBaseHullId() + "_glow2"), spriteCenter,
                new Vector2f(ship.getSpriteAPI().getWidth(), ship.getSpriteAPI().getHeight()), ship.getFacing() - 90f, colorToUse, true);
        ...
        if (phantomDelayCounter > PHANTOM_DELAY) {
            ...
            for (int i = 0; i < PHANTOM_FLICKER_CLONES; i++) {
                ...
                MagicRender.battlespace(Global.getSettings().getSprite(GfxCat.PHASE_GLOWS, ship.getHullSpec().getBaseHullId() + "_glow1"), modifiedPhantomPos, new Vector2f(0f, 0f),
