 ...
 */
public class MSS_OrionDeviceStats extends BaseShipSystemScript {

	private WeaponSlotAPI selectAppropriateSlot(ShipAPI ship) {
		WeaponSlotAPI rearmostSlot = null;   // Slot at the rear of the ship
		WeaponSlotAPI frontmostSlot = null;   // Slot at the front of the ship
		WeaponSlotAPI leftmostSlot = null;    // Slot at the left side of the ship
		WeaponSlotAPI rightmostSlot = null;   // Slot at the right side of the ship

		float maxRearDistance = Float.MAX_VALUE;
		float maxFrontDistance = Float.MIN_VALUE;
		float maxLeftDistance = Float.MAX_VALUE;
		float maxRightDistance = Float.MIN_VALUE;

		Vector2f shipFacingVector = VectorUtils.getDirectionalVector(ship.getLocation(), ship.getVelocity());
		Vector2f shipRightVector = VectorUtils.rotate(shipFacingVector, 90);

		for (WeaponSlotAPI slot : ship.getHullSpec().getAllWeaponSlotsCopy()) {
			if (slot.isSystemSlot()) {
				Vector2f slotPosition = slot.computePosition(ship);
				float distanceAlongFacing = Vector2f.dot(shipFacingVector, Vector2f.sub(slotPosition, ship.getLocation(), null));
				float distanceAlongRight = Vector2f.dot(shipRightVector, Vector2f.sub(slotPosition, ship.getLocation(), null));

				// Find the rearmost slot
				if (distanceAlongFacing < maxRearDistance) {
					maxRearDistance = distanceAlongFacing;
					rearmostSlot = slot;
				}

				// Find the frontmost slot
				if (distanceAlongFacing > maxFrontDistance) {
					maxFrontDistance = distanceAlongFacing;
					frontmostSlot = slot;
				}

				// Find the leftmost slot
				if (distanceAlongRight > maxRightDistance) {
					maxRightDistance = distanceAlongRight;
					rightmostSlot = slot;
				}

				// Find the rightmost slot
				if (distanceAlongRight < maxLeftDistance) {
					maxLeftDistance = distanceAlongRight;
					leftmostSlot = slot;
				}
			}
		}

		// Get the ship's behavior flags
		boolean isAcceleratingBackwards = ship.getEngineController().isAcceleratingBackwards();
		boolean isAccelerating = ship.getEngineController().isAccelerating();
		boolean isStrafingLeft = ship.getEngineController().isStrafingLeft();
		boolean isStrafingRight = ship.getEngineController().isStrafingRight();
		boolean isTurningLeft = ship.getEngineController().isTurningLeft();
		boolean isTurningRight = ship.getEngineController().isTurningRight();
		boolean isMovingForward = isAccelerating || isAcceleratingBackwards;

		// Precisely determine ship behavior and select the appropriate slot
		if (isAcceleratingBackwards) {
			return rearmostSlot;
		} else if (isMovingForward) {
			return frontmostSlot;
		} else if (isStrafingLeft && isTurningLeft) {
			return rightmostSlot;
		} else if (isStrafingRight && isTurningRight) {
			return leftmostSlot;
			...
			return rightmostSlot; // Adjust this to your preference
		} else if (isTurningRight) {
			return leftmostSlot; // Adjust this to your preference
		}

		// Default selection if none of the behaviors match
		return rearmostSlot;
	}
	...
	public static class OrionDeviceParams {
		...
		public float bombSpeed = 125f;
		public float bombInheritedVelocityFraction = 0.50f;
		...
		public String bombWeaponId = "MSS_orion_device";
	}
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		if (effectLevel >= 1 && !deployedBomb) {
			WeaponSlotAPI selectedSlot = selectAppropriateSlot(ship);
			if (selectedSlot != null) {
				spawnBomb(ship, selectedSlot);
			}
			deployedBomb = true;
