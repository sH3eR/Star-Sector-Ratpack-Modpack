...
public class MSS_oscillatingEffect implements BeamEffectPlugin 
{
    private float p = 0, random=0;
    private final float AMPLITUDE = 0.2f;
    ...
    public void advance(float amount, CombatEngineAPI engine, BeamAPI beam){
        ...
        if (beam.getBrightness()>0.1) {
            ...
            float offset = Math.min(p, 4) * AMPLITUDE * ((frequencyA*(frequencyB)) + (float)FastTrig.sin(20*random*p));
