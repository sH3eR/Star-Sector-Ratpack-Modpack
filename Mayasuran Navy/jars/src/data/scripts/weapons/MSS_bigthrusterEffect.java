...
public class MSS_bigthrusterEffect implements EveryFrameWeaponEffectPlugin {
            ...
            if(thrust>0){
                if(!turn){
                    ...
                    if(!accel){                        
                        ...
                        combinedAngle = MathUtils.clampAngle(combinedAngle + (1-clampedThrustToTurn)*MathUtils.getShortestRotation(NEUTRAL_ANGLE,accelerateAngle));
                        combinedAngle = MathUtils.clampAngle(combinedAngle + clampedThrustToTurn*MathUtils.getShortestRotation(NEUTRAL_ANGLE,turnAngle));
...
//                        engine.addFloatingText(weapon.getLocation(), " "+offAxis, 20, Color.yellow, SHIP, 1, 1);
    ...
    private void thrust(WeaponAPI weapon, float angle, float thrust, float smooth){
        ...
        float width=length*size.x/1+size.x/1;
